...
function AddIngrediente(props: AddIngredienteProps) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      axios
        .put(
          `http://127.0.0.1:5000/receitas/${props.receita_id}/ingredientes/${props.ingrediente}`,
          editData
        )
        .then((response) => response.data)
        .then(() => alert(`Ingrediente editado com sucesso.`))
  ...
  return (
    <>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed bg-yellow-100 py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[30rem] h-[26rem] shadow-lg shadow-black/25 overflow-auto">
          <Dialog.Title className="text-[#fc939a] uppercase font-bold text-center text-2xl font-serif p-2 border-[3px] border-[#fc939a] mb-10">
            Editar Ingrediente {props.ingrediente}
          </Dialog.Title>
